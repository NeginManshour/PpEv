...
# Example usage
output_subfolder_vina = 'Vina_TB_ex'
output_subfolder_vinardo = 'Vinardo_TB_ex'
    ...
    for file in excel_files:
        ...
        try:
            ...
            if len(sorted_data) >= 1000:
                sorted_data['vina_rank'] = range(1000)
            else:
                sorted_data['vina_rank'] = range(len(sorted_data))
...
# Example usage
input_subfolder = 'Vina_TB_ex'  # Subfolder name where the Excel files are located
output_subfolder = 'Vina_TB_ex_sorted'  # Subfolder name where sorted Excel files will be saved
...
# Example usage
input_subfolder = 'Vina_TB_ex_sorted'  # Subfolder name where the Excel files are located
output_subfolder = 'Vina_sorted_TB_alpha_rank'  # Subfolder name where processed Excel files will be saved
...
def copy_vina_sheets_to_dockq(dockq_dir, vina_dir, output_dir):
    ...
    vina_map = {extract_pdb_id_from_vina(f).upper(): f for f in vina_files if 'template_based' in f}
        ...
        if dockq_file.endswith('_TB.xlsx') and 'DockQ_data' in dockq_file:
            ...
            if pdb_id in vina_map:
                ...
                with pd.ExcelWriter(output_path, engine='openpyxl', mode='a') as writer:
                    vina_df.to_excel(writer, sheet_name='Vina', index=False)

                print(f'Updated {dockq_file} with Vina data for PDB ID {pdb_id}')
            else:
                print(f"No matching Vina file found for {dockq_file}")

# Example usage
dockq_dir = config['TB_uni']
vina_dir = os.path.join(config['output_directory'], 'Vina_sorted_TB_alpha_rank')
output_dir = os.path.join(config['output_directory'], 'vina_DockQ_TB')
...
def process_excel_files(directory_path, output_directory):
    ...
    for excel_file in excel_files:
        ...
        if 'Vina' not in xls.sheet_names:
            print(f'The file "{excel_file}" does not contain a "Vina" sheet.')
            continue

        dockq_df = pd.read_excel(excel_file_path, sheet_name='Sheet')
        vina_df = pd.read_excel(excel_file_path, sheet_name='Vina')

        pyrosetta_rank_column = 'vina_rank'
        alpha_to_dockq_map = dockq_df.set_index('AlphaFold Rank')['DockQ Rank'].to_dict()
        vina_df['DockQ Rank'] = vina_df['alphafold rank'].map(alpha_to_dockq_map)

        filtered_vina_df = vina_df.dropna(subset=['DockQ Rank'])

        spearman_corr, _ = spearmanr(filtered_vina_df[pyrosetta_rank_column], filtered_vina_df['DockQ Rank'])

        plt.figure(figsize=(10, 6))
        plt.scatter(filtered_vina_df['DockQ Rank'], filtered_vina_df[pyrosetta_rank_column], alpha=0.6)
        plt.title(f'File: {excel_file}\nSpearman Correlation: {spearman_corr:.2f}')
        plt.xlabel('DockQ Rank')
        plt.ylabel('vina_rank')
        plt.grid(True)
        plt.show()
    ...
    output_file_path = os.path.join(output_directory, 'correlations_vina_TB.xlsx')
...
# Example usage
directory_path = os.path.join(config['output_directory'], 'vina_DockQ_TB')
...
# Example usage
output_directory = config['Spearman_Correlation_directory_TB']
file_name = 'correlations_vina_TB.xlsx'
file_path = os.path.join(output_directory, file_name)

positive_percentage, negative_percentage = calculate_positive_negative_percentages(file_path)
print(f"Positive Vina_Spearman Correlation: {positive_percentage:.2f}%")
print(f"Negative Vina_Spearman Correlation: {negative_percentage:.2f}%")
...
for file_name in excel_files:
    ...
    # Load data from "Vina" sheet
    vina_df = pd.read_excel(file_path, sheet_name='Vina')
    structure_name_for_rank_zero = vina_df[vina_df['vina_rank'] == 0]['file'].iloc[0].replace('.pdb', '.pdb_clean')
    ...
    # DockQ score for the structure from Vina
    highest_vina_dockq_score = sheet_df[sheet_df['File Name'].str.contains(structure_name_for_rank_zero)]['DockQ'].max()

    # Calculate Loss
    score_Loss = highest_dockq_score - highest_vina_dockq_score

    # Append results
    results.append([pdb_id, highest_dockq_score, highest_vina_dockq_score, score_Loss])

# Create a DataFrame from the results
results_df = pd.DataFrame(results, columns=['File Name', 'DockQ', 'Vina ranked', 'Loss'])

# Save the results into a new Excel file
output_path = os.path.join(config['DockQ_Loss_directory_TB'], 'Vina_loss_TB.xlsx')
