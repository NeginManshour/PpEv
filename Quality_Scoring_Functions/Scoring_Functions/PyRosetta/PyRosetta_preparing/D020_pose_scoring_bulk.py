...
"""
Modified version of PyRosetta's D020_Pose_scoring.py 
Calculates pose scoring for a directory of PDBs and writes the resulting data to a TSV
file, sorted in ascending order (best to worst). The TSV file is saved in the parent 
of the provided PDB directory.
"""
...
def sort_key(pair):
    """
    Specify sorting criterion for pose scores
    """
    return(float(pair[1]))
...
# Args: path to PDB directory
arg_parser=ArgumentParser(description="Run Pose_scoring on many PDBs")
...
# Run Pose_scoring for each PDB file in model_dir
pdb_list = list(pdb_dir.iterdir())
scores = [] # List of pairs: (pdb name, score)
for pdb_file in pdb_list:
    if pdb_file.suffix == ".pdb":
        ...
        pose_from_file(pose, str(pdb_file))
        ...
        score = pose_scoring(pose, residues)
        scores.append((pdb_file.stem, score))
    
# Sort data and open output TSV file
scores.sort(reverse=False, key=sort_key)
out_path = pdb_dir.parent / "pyrosetta_scoring_7PRX_TB.tsv"
out_file = open(out_path, 'w')

# Write the data to TSV
out_file.write("#" + "  " + "old_rank" + "\t" + "structure_name" + "\t" + "\t" + "score" + "\n")
for i in range(len(scores)):
    ...
    file_split = pair[0].split("_")
    file_split1 = file_split[1].split(".")
    af_rank = int(file_split1[0]) # AlphaFold rank

    out_file.write(str(af_rank) + "\t")
    out_file.write(pair[0] + "\t") # Original structure name
    out_file.write(str(pair[1]) + "\n") # Score

out_file.close()
print("pose scoring data written")
