...
# This program runs DockQ on all PDB files in a given directory and saves the data to an Excel spreadsheet. 
...
#    exec_path=os.path.dirname(os.path.abspath(sys.argv[0]))    
    ...
    fnat_out1 = os.popen(cmd_fnat)
    fnat_out = fnat_out1.read()
...
#    print atoms_def_in_both
    ...
    # Calculate IOU
    IOU = nat_correct/(nat_total + model_total - nat_correct)
    info['IOU'] = IOU
...
# Run DockQ on a single PDB
def dockq_single(model_file, args):
...
sheet["B1"] = "Capri"
sheet["C1"] = "Best Info"
sheet["D1"] = "Eq. res. in chain " + info['chain1']
sheet["E1"] = "Eq. res. in chain " + info['chain2']
sheet["F1"] = "Fnat"
sheet["G1"] = "Fnonnat"
sheet["H1"] = "iRMS"
sheet["I1"] = "LRMS"
...
sheet["K1"] = "IOU"
"""
# Write the calculated info for a given PDB file to an Excel spreadsheet
def write_xl(xl_sheet, row, filename, info):

    file_split = filename.split("_")
    file_split1 = file_split[1].split(".")
    af_rank = int(file_split1[0]) # AlphaFold rank

    xl_sheet["A"+row] = filename
    xl_sheet["B"+row] = af_rank
    xl_sheet["C"+row] = info['capri']
    xl_sheet["D"+row] = info['len1']
    xl_sheet["E"+row] = info['len2']
    xl_sheet["F"+row] = info['fnat']
    xl_sheet["G"+row] = info['fnonnat']
    xl_sheet["H"+row] = info['irms']
    xl_sheet["I"+row] = info['Lrms']
    xl_sheet["J"+row] = info['IOU']
    xl_sheet["K"+row] = info['DockQ']
...
def main():
    ...
    parser.add_argument('model_dir',metavar='<model_dir>',type=str,nargs=1,help='path to directory containing model files')
    parser.add_argument('native',metavar='<native>',type=str,nargs=1,help='path to native file')
    parser.add_argument('-xl',metavar='xl', default='', type=str,nargs=1, help='path to Excel file to save data')
...
#    parser.add_argument('-comb',default=False,action='store_true',help='use all cyclicchain permutations to find maximum DockQ (number of comparisons is n!*m! = 24*24 = 576 for two tetramers interacting')
    ...
    model_dir = Path(args.model_dir[0])

    xl_path = None
    if args.xl != "":
        xl_path = Path(args.xl[0])

    # Open an Excel file to write, add column labels
    workbook = Workbook()
    if xl_path != None: 
        workbook = load_workbook(str(xl_path))
    sheet = workbook.active
    sheet["A1"] = "File Name" # TODO: consider changing "file name" to "rank"
    sheet["B1"] = "AlphaFold Rank"
    sheet["C1"] = "Capri"
    sheet["D1"] = "Eq. res. in chain 1" 
    sheet["E1"] = "Eq. res. in chain 2" 
    sheet["F1"] = "Fnat"
    sheet["G1"] = "Fnonnat"
    sheet["H1"] = "iRMS"
    sheet["I1"] = "LRMS"
    ...
    row_index = 2
    dockq_scores = [] # List of pairs: (row number, DockQ score)
    for model_file in model_list:
        if model_file.suffix == ".pdb":
            filename = str(model_file.stem)
            info = dockq_single(model_file, args)
            dockq_scores.append((row_index, info['DockQ']))
            write_xl(sheet, str(row_index), filename, info) # Write to Excel
            row_index += 1

    # Write DockQ ranks to the Excel file
    dockq_scores.sort(reverse=True, key=sort_key)
    for i in range(len(dockq_scores)):
        pair = dockq_scores[i]
        sheet["L"+str(pair[0])] = i

    # Save Excel workbook
    if xl_path == None:
        native_path = Path(args.native[0])
        protein_name = (native_path.stem).split("_")[0]
        workbook_path = model_dir.parent / (protein_name + "_DockQ_data.xlsx")
        workbook.save(filename = str(workbook_path))
    else:
